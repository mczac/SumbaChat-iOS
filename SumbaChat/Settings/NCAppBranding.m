...
+ (UIImage *)navigationLogoImage
{
    ...
    // Keep as template — UIGraphics scaling of a template image bakes it black, which
    // then stays dark on the blue nav bar when forced to AlwaysOriginal.
    UIImage *waveLogo = [UIImage imageNamed:@"sumbaLoginLogo"];
    if (waveLogo) {
        return [waveLogo imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
    }
