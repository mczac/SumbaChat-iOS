...
public class NCSettingsController: NSObject {
    ...
    /// Soft/hard SumbaChat App Store update (from `config.sumbachat-client`).
    private var sumbaChatUpdateAlertController: UIAlertController?
    ...
    override init() {
        ...
        self.mirrorAccountTokensForExtensions()
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(sumbaChatClientUpdateCheck), name: .SumbaChatClientUpdateCheck, object: nil)
    }
    ...
    private func mirrorAccountTokensForExtensions() {
        let accountIds = NCDatabaseManager.sharedInstance().allAccounts().map(\.accountId)
        NCKeyChainController.sharedInstance().mirrorStoredTokens(forAccountIds: accountIds)
    }
    ...
    // MARK: - SumbaChat client update (capabilities `sumbachat-client`)

    @objc private func sumbaChatClientUpdateCheck() {
        let ui = NCUserInterfaceController.sharedInstance()
        // Capabilities can arrive before the split UI exists (cold login).
        guard ui.mainViewController != nil else {
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) { [weak self] in
                self?.sumbaChatClientUpdateCheck()
            }
            return
        }

        // Prefer Talk's HTTP 426 hard gate if it is already on screen.
        if self.updateAlertController != nil,
           self.updateAlertController === ui.mainViewController.presentedViewController {
            return
        }

        guard let kind = SumbaChatClientConfig.pendingUpdateKind() else {
            return
        }

        // Don't stack another soft alert while one is already presented.
        if let existing = self.sumbaChatUpdateAlertController,
           existing === ui.mainViewController.presentedViewController {
            return
        }

        let alert: UIAlertController
        switch kind {
        case .mandatory(let policy):
            alert = self.makeSumbaChatUpdateAlert(mandatory: true, policy: policy)
        case .recommended(let policy):
            alert = self.makeSumbaChatUpdateAlert(mandatory: false, policy: policy)
        }

        self.sumbaChatUpdateAlertController = alert
        NCUserInterfaceController.sharedInstance().presentAlertIfNotPresentedAlready(alert)
    }

    private func makeSumbaChatUpdateAlert(mandatory: Bool, policy: SumbaChatClientConfig.UpdatePolicy) -> UIAlertController {
        let title = NSLocalizedString("SumbaChat update is available", comment: "App update alert title")
        let message = mandatory
            ? NSLocalizedString("Please update to continue using application", comment: "Mandatory app update alert message")
            : NSLocalizedString("We recommend you to update", comment: "Recommended app update alert message")

        let alert = UIAlertController(title: title, message: message, preferredStyle: .alert)

        if !mandatory {
            let cancel = UIAlertAction(title: NSLocalizedString("Cancel", comment: ""), style: .cancel) { [weak self] _ in
                SumbaChatClientConfig.dismissRecommended(for: policy)
                self?.sumbaChatUpdateAlertController = nil
            }
            alert.addAction(cancel)
        }

        let update = UIAlertAction(title: NSLocalizedString("Update", comment: ""), style: .default) { [weak self] _ in
            if let url = policy.appStoreURL {
                UIApplication.shared.open(url, options: [:], completionHandler: nil)
            }
            // Mandatory: keep a reference and re-check soon so the alert returns if they bounce back.
            if mandatory {
