...
class SettingsTableViewController: UITableViewController, UITextFieldDelegate, UserStatusViewDelegate, CallsFromOldAccountViewControllerDelegate, DetailedOptionsSelectorTableViewControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        // Settings apply immediately — dismiss is Close, not Cancel.
        self.cancelButton.title = NSLocalizedString("Close", comment: "Dismiss settings sheet")
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        // Standard pop deselect — keeps the highlight in sync with the interactive transition.
        ...
        // Refresh Caching subtitle after the pop finishes. Reloading the section during the
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case SettingsSection.kSettingsSectionDebug.rawValue:
            ...
            if indexPath.row == 1 {
                ...
                cell.textLabel?.text = NSLocalizedString("Compression rules", comment: "Debug: compression rules reference")
                cell.detailTextLabel?.text = nil
                cell.accessoryType = .disclosureIndicator
            ...
            cell.textLabel?.text = NSLocalizedString("Caching rules", comment: "Debug: caching rules reference")
            cell.detailTextLabel?.text = nil
            cell.accessoryType = .disclosureIndicator
...
extension SettingsTableViewController {
    ...
    /// Updates Advanced → Caching detail text without reloading the section (avoids pop glitches).
    private func refreshCachingRowSubtitle() {
        updateCacheUsageSizes()
        let sections = getSettingsSections()
        guard let section = sections.firstIndex(of: SettingsSection.kSettingsSectionAdvanced.rawValue) else { return }
        let options = getAdvancedSectionOptions()
        guard let row = options.firstIndex(of: AdvancedSectionOption.kAdvancedSectionOptionCaching.rawValue) else { return }
