...
@objcMembers public class InputbarViewController: SLKTextViewController, NCChatTitleViewDelegate {
    ...
    private var messageTextChromeEffectView: UIVisualEffectView?
    private var previousNavigationBarTintColor: UIColor?
    private var previousNavigationBarTranslucent: Bool?
    private var previousNavigationBarStandardAppearance: UINavigationBarAppearance?
    private var previousNavigationBarScrollEdgeAppearance: UINavigationBarAppearance?
    private var previousNavigationBarCompactAppearance: UINavigationBarAppearance?
    private var previousNavigationBarCompactScrollEdgeAppearance: UINavigationBarAppearance?
    ...
    public override func viewDidLoad() {
        ...
        // Match rooms search / compact composer height (taller field than stock SLK insets).
        self.textInputbar.contentInset = .init(top: 10, left: 10, bottom: 10, right: 10)
        self.textView.textContainerInset = .init(top: 11, left: 14, bottom: 11, right: 14)
        ...
        // Chat uses a light/system nav bar. Skip branded styleViewController — it sets
        // themeTextColor (often white) on the shared UINavigationBar and makes the back chevron invisible.
        self.styleChatNavigationOverlay()
    ...
    public override func viewDidAppear(_ animated: Bool) {
        ...
        // Transition can re-apply the rooms-list white tint after viewWillAppear.
        self.applyChatNavigationBarChrome()
    }

    public override func viewWillDisappear(_ animated: Bool) {
        super.viewWillDisappear(animated)
        // Only restore when leaving chat — presenting a sheet also triggers disappear.
        if isMovingFromParent || isBeingDismissed {
            self.restoreNavigationBarChromeIfNeeded()
        }
    }
    ...
    public override func traitCollectionDidChange(_ previousTraitCollection: UITraitCollection?) {
        if self.traitCollection.hasDifferentColorAppearance(comparedTo: previousTraitCollection) {
            self.textView.tintColor = UIColor(cgColor: UIColor.systemBlue.cgColor)
            // Border uses CGColor — refresh chrome + stroke for the new appearance.
            self.styleMessageInputChrome()
            self.styleChatNavigationOverlay()
    ...
    /// Frosted pill field + circular action buttons (rooms-search style), not solid gray fill.
    func styleMessageInputChrome() {
        self.styleFloatingComposerChrome()

        let stroke = Self.messageComposerChromeStroke.cgColor

        // Clear field + sibling blur/glass (UITextView cannot host the effect reliably).
        ...
        self.ensureMessageTextChromeEffect()
        if let messageTextView = self.textView as? NCMessageTextView {
            messageTextView.placeholderColor = .secondaryLabel
        }

        self.styleMessageInputActionButton(self.leftButton, stroke: stroke)
        self.styleMessageInputActionButton(self.rightButton, stroke: stroke)
        self.updateMessageInputChromeCorners()
    }

    private func styleMessageInputActionButton(_ button: UIButton, stroke: CGColor) {
        button.tintColor = .label
        button.clipsToBounds = true
        button.layer.borderWidth = 1.0 / UIScreen.main.scale
        button.layer.borderColor = stroke
        button.installSumbaChromeEffect()
    }

    private func ensureMessageTextChromeEffect() {
        if messageTextChromeEffectView == nil {
            ...
            messageTextChromeEffectView = effectView
        }

        guard let effectView = messageTextChromeEffectView else { return }

        if effectView.superview !== self.textInputbar {
            self.textInputbar.insertSubview(effectView, belowSubview: self.textView)
        }
    }
    ...
    private func updateMessageInputChromeCorners() {
        ...
        if let effectView = messageTextChromeEffectView {
            effectView.frame = self.textView.frame
            effectView.layer.cornerRadius = self.textView.layer.cornerRadius
        }
        ...
        for button in [self.leftButton, self.rightButton] {
            ...
            // Keep frosted effect clipped to the pill.
            button.viewWithTag(9_140_277)?.layer.cornerRadius = button.layer.cornerRadius
            button.viewWithTag(9_140_277)?.clipsToBounds = true
        }
    }

    /// Dedicated material overlay so title/subtitle contrast stays stable over scrolling messages.
    func styleChatNavigationOverlay() {
        ...
        // Back chevron + bar buttons must not inherit branded white tint from the rooms list.
        let barButton = UIBarButtonItemAppearance()
        barButton.normal.titleTextAttributes = [.foregroundColor: UIColor.label]
        barButton.highlighted.titleTextAttributes = [.foregroundColor: UIColor.label]
        appearance.buttonAppearance = barButton
        appearance.backButtonAppearance = barButton
        appearance.doneButtonAppearance = barButton

        // Tint alone is not enough after a blue/white rooms bar — pin a label-colored chevron.
        ...
        self.applyChatNavigationBarChrome(appearance: appearance)
    }

    private func applyChatNavigationBarChrome(appearance: UINavigationBarAppearance? = nil) {
        guard let navigationBar = self.navigationController?.navigationBar else { return }

        if previousNavigationBarTintColor == nil {
            previousNavigationBarTintColor = navigationBar.tintColor
            previousNavigationBarTranslucent = navigationBar.isTranslucent
            previousNavigationBarStandardAppearance = navigationBar.standardAppearance
            previousNavigationBarScrollEdgeAppearance = navigationBar.scrollEdgeAppearance
            previousNavigationBarCompactAppearance = navigationBar.compactAppearance
            if #available(iOS 15.0, *) {
                previousNavigationBarCompactScrollEdgeAppearance = navigationBar.compactScrollEdgeAppearance
            }
        }

        navigationBar.isTranslucent = true
        // Adaptive contrast on the light chat bar (overrides rooms-list themeTextColor / white).
    ...
    private func restoreNavigationBarChromeIfNeeded() {
        guard let navigationBar = self.navigationController?.navigationBar else { return }

        if let tintColor = previousNavigationBarTintColor {
            navigationBar.tintColor = tintColor
        }
        if let isTranslucent = previousNavigationBarTranslucent {
            navigationBar.isTranslucent = isTranslucent
        }
        if let standard = previousNavigationBarStandardAppearance {
            navigationBar.standardAppearance = standard
        }
        if let scrollEdge = previousNavigationBarScrollEdgeAppearance {
            navigationBar.scrollEdgeAppearance = scrollEdge
        }
        if let compact = previousNavigationBarCompactAppearance {
            navigationBar.compactAppearance = compact
        }
        if #available(iOS 15.0, *), let compactScrollEdge = previousNavigationBarCompactScrollEdgeAppearance {
            navigationBar.compactScrollEdgeAppearance = compactScrollEdge
        }

        previousNavigationBarTintColor = nil
        previousNavigationBarTranslucent = nil
        previousNavigationBarStandardAppearance = nil
        previousNavigationBarScrollEdgeAppearance = nil
        previousNavigationBarCompactAppearance = nil
        previousNavigationBarCompactScrollEdgeAppearance = nil
    }
