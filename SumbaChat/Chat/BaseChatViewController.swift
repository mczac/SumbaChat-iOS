...
@objcMembers public class BaseChatViewController: InputbarViewController,
    ...
    /// Height covered by composer (+ keyboard when raised), from chrome top to the view bottom.
    func geometricComposerOverlayHeight() -> CGFloat {
        var chromeTop = self.textInputbar.convert(CGPoint.zero, to: self.view).y

        if let replyMessageView, replyMessageView.isVisible {
            chromeTop = min(chromeTop, replyMessageView.convert(CGPoint.zero, to: self.view).y)
        }

        if self.autoCompletionView.bounds.height > 0.5 {
            chromeTop = min(chromeTop, self.autoCompletionView.convert(CGPoint.zero, to: self.view).y)
        }

        return max(0, self.view.bounds.height - chromeTop)
    }

    /// Bottom inset used for “at end” math: keyboard + input/mic obstruction, not merely the screen edge.
    func effectiveChatBottomInset() -> CGFloat {
        let geometric = self.geometricComposerOverlayHeight()
        guard let tableView else { return geometric }
        // SLK sets contentInset.bottom from composer geometry (adjustment = never).
        return max(tableView.contentInset.bottom, geometric)
    }

    /// Distance above the inset-aware chat bottom (composer / mic / keyboard). Near zero means last content clears the chrome.
    func distanceFromChatBottom() -> CGFloat {
        guard let tableView else { return 0 }

        let height = tableView.bounds.height
        guard height > 1 else { return 0 }

        let bottomInset = self.effectiveChatBottomInset()
        let topInset = tableView.adjustedContentInset.top
        var maxOffsetY = tableView.contentSize.height - height + bottomInset
        let minOffsetY = -topInset
        if maxOffsetY < minOffsetY {
            maxOffsetY = minOffsetY
        }

        return maxOffsetY - tableView.contentOffset.y
    }

    func isNearChatBottom(threshold: CGFloat) -> Bool {
        distanceFromChatBottom() <= threshold
    }

    /// Keep the last message above the input when the keyboard shows/hides, if we were already at the end.
    public override func didChangeKeyboardStatus(_ status: SLKKeyboardStatus) {
        let wasNearBottom = self.isNearChatBottom(threshold: 48)
    ...
    public func updateToolbar(animated: Bool) {
        ...
        let animations = {
            if !self.isNearChatBottom(threshold: 10) {
    ...
    public override func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        // Live chevron: hide only once the last message clears the input chrome, not the screen edge.
        self.updateToolbar(animated: false)
    }
    ...
    func shouldScrollOnNewMessages() -> Bool {
        guard self.isVisible, self.tableView != nil else { return false }

        // Follow new messages when within 80pt of the composer-aware bottom.
        return self.isNearChatBottom(threshold: 80)
    }
