...
class NCAPIController: NSObject, NKCommonDelegate {
    ...
    public func setupNCCommunication(forAccount account: TalkAccount) {
        guard let token = NCKeyChainController.sharedInstance().token(forAccountId: account.accountId) else {
            NCLog.log("setupNCCommunication: missing token for \(account.accountId)")
            return
        }
        guard let serverCapabilities = NCDatabaseManager.sharedInstance().serverCapabilities(forAccountId: account.accountId) else {
            NCLog.log("setupNCCommunication: missing serverCapabilities for \(account.accountId)")
            return
        }
...
"""
    ...
    func uniqueNameForFileUpload(withName fileName: String, isOriginalName: Bool, forAccount account: TalkAccount, completionBlock: @escaping (_ fileServerURL: String?, _ fileServerPath: String?, _ errorCode: Int, _ errorDescription: String?) -> Void) {
        ...
        guard let fileServerPath = self.serverFilePath(forFileName: fileName, forAccount: account),
            ...
            NCLog.log("uniqueNameForFileUpload: missing server path/URL for \(fileName) account=\(account.accountId)")
            completionBlock(nil, nil, -1, NSLocalizedString("Couldn't prepare the upload. Open SumbaChat and try again.", comment: "User-facing upload path error"))
            return
        }
        ...
        NextcloudKit.shared.readFileOrFolder(serverUrlFileName: fileServerURL, depth: "0", showHiddenFiles: true, includeHiddenFiles: [], requestBody: nil, options: options) { _, files, _, error in
            ...
            } else if error.errorCode == 404 || (error.errorCode == 0 && files.isEmpty) {
                // Missing remote object (404), or 2xx PROPFIND that parsed to no files — treat as available.
                completionBlock(fileServerURL, fileServerPath, 0, nil)
            } else {
                let detail = error.errorDescription.isEmpty
                    ? "PROPFIND failed (code=\(error.errorCode), files=\(files.count)) url=\(fileServerURL)"
                    : "\(error.errorDescription) url=\(fileServerURL)"
                NCLog.log("uniqueNameForFileUpload: \(detail)")
                // UI gets a human message only — never server URLs or DAV jargon.
                completionBlock(nil, nil, error.errorCode, Self.userFacingFileUploadError(code: error.errorCode))
            }
        }
    }

    /// Safe strings for alerts. Technical detail belongs in logs only.
    static func userFacingFileUploadError(code: Int) -> String {
        switch code {
        case 429:
            return NSLocalizedString("The server is temporarily limiting requests. Wait a moment and try again.", comment: "HTTP 429 during media upload")
        case 401, 403:
            return NSLocalizedString("You don't have permission to upload. Try signing in again in SumbaChat.", comment: "HTTP 401/403 during media upload")
        case 404, 409:
            return NSLocalizedString("Couldn't prepare the upload folder. Try again.", comment: "HTTP 404/409 during media upload")
        case 413, 507:
            return NSLocalizedString("Not enough storage space on the server.", comment: "HTTP 413/507 during media upload")
        default:
            return NSLocalizedString("Upload failed. Try again.", comment: "Generic media upload failure")
        }
    }
