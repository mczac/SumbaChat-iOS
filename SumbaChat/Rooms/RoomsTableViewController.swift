...
class RoomsTableViewController: UITableViewController, CCCertificateDelegate, UISearchBarDelegate, UISearchControllerDelegate, UISearchResultsUpdating, UserStatusViewDelegate {
    ...
    private func configureRightBarButtonItems() {
        ...
        // iOS 26: plain + (system label color), brand accent lives on the nav logo.
        if #available(iOS 26.0, *) {
            newConversationButton?.image = UIImage(systemName: "plus")
            newConversationButton?.style = .plain
            newConversationButton?.tintColor = .label
            newConversationButton?.hidesSharedBackground = true
    ...
    private func updateProfileButtonImage() {
        let activeAccount = NCDatabaseManager.sharedInstance().activeAccount()
        // Blue branded nav bar does not flip with dark mode — always use the light avatar
        // (white circle / dark initials), matching filter/plus tint on the same bar.
        if var profileImage = NCAPIController.sharedInstance().userProfileImage(forAccount: activeAccount, withStyle: .light) {
            ...
            // Increase the profile image size to leave space for the status (light ring on blue bar).
            let ringColor = UIColor.white.withAlphaComponent(0.35)
            profileImage = profileImage.withCircularBackground(backgroundColor: ringColor, diameter: 32.0, padding: 1.0) ?? profileImage
            ...
            if activeUserStatus?.hasVisibleStatusIcon() == true {
                let statusRing = NCAppBranding.themeColor()
                if #available(iOS 26.0, *) {
                    ...
                    statusImage = activeUserStatus?.getSFUserStatusIcon()?.withCircularBackground(backgroundColor: statusRing, diameter: 14.0, padding: 1.0)
                }
            }
